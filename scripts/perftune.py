...
#### Private methods ############################
    ...
    def __mlx_irq_to_queue_idx(self, irq):
        ...
        mama_fp_irq_re = re.compile(r"mana_q(\d+)")
        ...
        elif mama := mama_fp_irq_re.search(self.__irqs2procline[irq]):
            perftune_print("fruch, forund mama", self.__irqs2procline[irq])
            return int(mama.group(1))
        return sys.maxsize
    ...
    def __get_irq_to_queue_idx_functor(self, iface):
        ...
        perftune_print ("Driver name for iface {} is {}".format(iface, driver_name))
